-- 为所有订单生成完整的物流轨迹数据
-- 此脚本将为所有订单生成符合实际且可以在地图上展示的物流轨迹

-- 创建一个函数来生成两点之间的中间点
CREATE OR REPLACE FUNCTION generate_intermediate_point(
...
-- 主函数：为订单生成物流轨迹
CREATE OR REPLACE FUNCTION generate_complete_order_trajectories() 
RETURNS void AS $$
DECLARE
    order_record RECORD;
...
BEGIN
    ...
    LOOP
        -- 删除该订单的旧轨迹（如果有）
        DELETE FROM logistics_trajectories WHERE order_id = order_record.id;
        ...
        -- 分拣中心位置（商家附近的一个点）
        sorting_center_location := generate_intermediate_point(
            shop_location, 
        ...
        local_distribution_location := generate_intermediate_point(
...
-- 执行存储过程生成轨迹
SELECT generate_complete_order_trajectories();
...
-- 删除临时存储过程
DROP FUNCTION generate_complete_order_trajectories();
DROP FUNCTION generate_intermediate_point;

SELECT '✅ 所有订单物流轨迹数据生成完成！' as message;